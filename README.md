...
Third Normal Form (3NF) Compliance: All tables are structured to eliminate redundancy. For example, in the staff table, all descriptive fields depend strictly on the primary key staff_id, removing all transitive dependencies.

Many-to-Many (M:N) Resolution: The relationship between orders and menu_items is managed via the order_items junction table. This tracking table features a composite primary key (order_id, item_id) to handle quantities per item effortlessly.

Automated Mathematical Precision:

Decimal values for hours_worked in shifts are calculated automatically from system timestamps.
...
Execution Steps
Open DBeaver and connect to your PostgreSQL database server.

Open the script file 02_finalll.sql (File -> Open File...).

Make sure your active connection dropdown at the top right of the script editor points to your PostgreSQL server.

Execute the full script by clicking the Execute SQL Script button on the left toolbar (or press Alt + X / Fn + Alt + X).
