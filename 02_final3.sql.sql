...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE 'CREATE DATABASE restaurant_db';
    END IF;
END $$;
...
-- ===== PART 3: ALTER TABLE CONSTRAINTS & DEFAULTS =====
...
WITH new_menu AS (
    ...
    SELECT (SELECT category_id FROM restaurant_schema.categories WHERE name = 'Beverages') AS category_id, 'Shubat Brew' AS name, 1500.00 AS price, true AS is_available, 160 AS calories UNION ALL
...
WITH new_orders AS (
    ...
    SELECT (SELECT table_id FROM restaurant_schema.tables WHERE table_number = 101), (SELECT staff_id FROM restaurant_schema.staff WHERE phone = '+77716689185'), '2026-06-03 20:00:00', 'Paid', 3500.00 UNION ALL
...
-- ===== PART 5: DATA UPDATES & TRANSACTIONS =====
...
-- ===== PART 7: ROLES & SECURITY (GRANT) =====

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'restaurant_readonly') THEN
        CREATE ROLE restaurant_readonly LOGIN PASSWORD 'readonly_password';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'restaurant_writer') THEN
...
revoke delete on all tables in schema restaurant_schema from restaurant_writer;